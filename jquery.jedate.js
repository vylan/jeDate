...
(function (root, factory) {
	...
	function jeDate(elem, opts) {
		...
		this.unbind = function () {
			var jetrigger = opts.trigger != undefined ? opts.trigger : config.trigger;
			if (jetrigger) {
				that.valCell.unbind(jetrigger);
			}
		};
	}
	...
	jedfn.initdates = function () {
		...
		if (isinitVal && jetrigger) {
			...
			that.setValue(reVal[0], opts.format || config.format, false);
		}
		//判断固定元素是否存在
		if (!isShow) {
	...
	jedfn.renderHtml = function (ys, ms, ds, opts, boxcls) {
		...
		if (isToday) {
			btnStr += '<span class="today">' + lang.today + '</span>'
		}
		btnStr += '<span class="setok">' + lang.yes + '</span>';
		btnsDiv = $("<div/>", {
			"class": "btnscon"
		}).append(btnStr);
		...
		if (mlens == 7) {
			...
			that.maincon(".ymsbox", 0).append('<em class="fa fa-angle-double-left yearprev yprev"></em><em class="fa fa-angle-double-right yearnext ynext"></em><em class="close iconfont icon-gs-close"></em>').addClass("ymfix");
			//将生成的年月插入到元素中
			that.eachYM(vals.YYYY, vals.MM, opts, boxCell, ".fixcon");
		}
		//为年月的情况下执行
		if (mlens == 1 || mlens == 2) {
			that.maincon(".ymsbox", 0).append('<em class="fa fa-angle-double-left yprev"></em><em class="fa fa-angle-double-right yearnext ynext"></em>');
	...
	jedfn.createYMHtml = function (ys, ms, opts) {
		...
		var ymCls = $("<p/>").css({
				"width": jet.isBool(opts.multiPane) ? '' : '50%'
			}),
	...
	jedfn.eachYM = function (y, m, opts, boxCell, clsCell) {
		...
		$.each(new Array(multiPane ? 1 : 2), function (s) {
			...
			$.each(ymArr, function (i, val) {
				table.find("td").eq(i).addClass(val.style).attr({
					idx: val.idx,
					"je-val": val.ym
				}).html(val.ym)
			});
		...
		ymstit.append("<p>" + sval + " ~ " + eval + "</p>").attr({
			min: mnx[0],
			max: mnx[1]
		});
	};
